...
class EditAdjust extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    componentDidMount() {
        ...
        axios.get('/api/products/adjustments/search?adjustId=' + this.props.match.params.adjustmentId, { headers: { "Authorization": "Bearer " + loginToken } })
            .then((resp) => {
              console.log(resp)
                let adjust = resp.data.response[0]
                this.setState({
                    adjustId: adjust.adjustId,
                    orderDate: adjust.orderDate,
                    memo: adjust.memo
                }, this.getAdjustBatch)
            }).catch((err) => {
                console.error(err)
    ...
    getAdjustBatch() {
        const loginToken = window.localStorage.getItem("token");
        axios.get('/api/products/adjustments/search?batchId=' + this.state.adjustId, { headers: { "Authorization": "Bearer " + loginToken } })
            .then((resp) => {
              console.log(resp)
                this.setState({
    ...
    async submitProductOrder() {
        ...
        if (window.confirm(
            ...
            for (var i = 0; i < this.state.productList.length; i++) {
                ...
                axios.put('/api/products/update?id=' + product.ProductId + '&quantity=' + newQuantity + '&name=' + product.name,
                ...
                try {
                    const response = await axios.put('/api/products/adjustments/update?id=' + product.id + '&orderDate=' + this.state.orderDate + '&orderId=' + this.state.orderId +
