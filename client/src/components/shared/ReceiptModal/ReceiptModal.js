...
class ReceiptModal extends Component {
  ...
  componentDidUpdate() {
    if (this.props.state.now && this.state.receipt == true) {
      this.setState({
        receipt: false
      })
      ...
      html2canvas(input)
        .then((canvas) => {
          ...
          const pdf = new jsPDF();
          pdf.addImage(imgData, 'JPEG', 0, 0, 0, 297);
          pdf.addImage(logoData, 'PNG', 30, 10, 20, 20);
          pdf.autoPrint();
          var blob = pdf.output('blob');
          var blobFile = new File([blob], "filename.pdf", { type: 'application/pdf' });
          var data = blobFile;
  ...
  getSignedRequest(file) {
    ...
    xhr.open('GET', `/api/receipts/sign-s3?file-name=testFile.pdf&file-type=application/pdf&scriptId=` + this.props.state.id + '&now=' + this.props.state.now + '&name=' + this.props.state.patientName + '&date=' + date,
